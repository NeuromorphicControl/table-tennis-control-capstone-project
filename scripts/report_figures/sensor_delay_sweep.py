...
def render_report_figure(points: list[SweepPoint], timestep: float, output_base: Path, formats: list[str], dpi: int, title: bool, strike_rate_floor: float) -> list[Path]:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--delay-step", type=int, default=20, help="ticks between consecutive delay values (default: 20)")
    parser.add_argument("--max-delay", type=int, default=200, help="largest delay to test if the strike rate never collapses, in ticks (default: 200)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    saved = render_report_figure(points, timestep, args.output, args.formats, args.dpi, title=not args.no_title, strike_rate_floor=args.strike_rate_floor)
    for path in saved:
        print(f"wrote {path}")
    return 0
